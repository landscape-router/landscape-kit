...
machine_shell 'echo "== S-3 pgrep: $(pgrep -a -f "^/usr/local/bin/lkit --non-interactive uninstall" || echo none)"; pgrep -f "^/usr/local/bin/lkit --non-interactive uninstall" | head -1 | xargs -r kill -INT || true'
