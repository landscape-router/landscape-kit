...
machine_shell 'for i in $(seq 1 200); do REQUEST=$(ls /run/lkit/operations/*.request.json 2>/dev/null | head -1); [ -n "$REQUEST" ] && break; sleep 0.1; done; echo "== S-3 request=[$REQUEST]"; test -n "$REQUEST"; CANCEL=$(printf "%s" "$REQUEST" | sed "s/\.request\.json$/.cancel/"); echo "== S-3 cancel file: [$CANCEL]"; touch "$CANCEL"'
