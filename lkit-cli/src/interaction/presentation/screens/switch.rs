...
impl OperationScreen for SwitchScreen {
    fn result_key(&self, result: OperationResult) -> &'static str {
        match result {
            OperationResult::Success => crate::keys::PRESENTATION_SWITCH_COMPLETE,
            OperationResult::Failed => crate::keys::PRESENTATION_SWITCH_FAILED,
            OperationResult::Cancelled => crate::keys::PRESENTATION_SWITCH_CANCELLED,
        }
    }
    ...
    fn render(
        ...
        let header_text = match result {
            Some(OperationResult::Success) => {
                crate::tr!(crate::keys::PRESENTATION_SWITCH_COMPLETE)
            }
            Some(OperationResult::Failed) => {
                crate::tr!(crate::keys::PRESENTATION_SWITCH_FAILED)
            }
            Some(OperationResult::Cancelled) => {
                crate::tr!(crate::keys::PRESENTATION_SWITCH_CANCELLED)
            }
            None => crate::tr!(crate::keys::PRESENTATION_OPERATION_SWITCH),
        };
        frame.render_widget(
