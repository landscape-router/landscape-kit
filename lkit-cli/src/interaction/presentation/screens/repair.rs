...
pub(crate) struct RepairScreen;

impl OperationScreen for RepairScreen {
    fn result_key(&self, result: OperationResult) -> &'static str {
        match result {
            OperationResult::Success => crate::keys::PRESENTATION_REPAIR_COMPLETE,
            OperationResult::Failed => crate::keys::PRESENTATION_REPAIR_FAILED,
            OperationResult::Cancelled => crate::keys::PRESENTATION_REPAIR_CANCELLED,
        }
    }
    ...
    fn render(
        ...
        let header_text = match result {
            Some(OperationResult::Success) => {
                crate::tr!(crate::keys::PRESENTATION_REPAIR_COMPLETE)
            }
            Some(OperationResult::Failed) => {
                crate::tr!(crate::keys::PRESENTATION_REPAIR_FAILED)
            }
            Some(OperationResult::Cancelled) => {
                crate::tr!(crate::keys::PRESENTATION_REPAIR_CANCELLED)
            }
            None => crate::tr!(crate::keys::PRESENTATION_OPERATION_REPAIR),
        };
        frame.render_widget(
