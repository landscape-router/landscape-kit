...
终端小于 72x18 时不渲染骨架,居中显示"终端尺寸过小（至少需要 72x18）"提示与
`Landscape Kit` 边框标题(提示文案包含具体阈值,用户可直接得知需要放大到多少)。
全屏页(网络向导、阻塞接管屏、委托操作页)无侧栏,自行定义
header/body/footer。
...
- 状态行:当前 notice,按级别染色——`Ready` 灰、过程信息/引导提示黄、操作成功
  ...
  `[L] 切换到 English (en)`),按 `L` 切换到所示目标;文本编辑中显示当前语言
...
- 确认层:居中,先 `Clear` 再带边框标题渲染;Enter 确认、Esc 关闭;
  首行问题加粗,`Esc` 取消行灰色;
- 进度弹层:居中;Esc 关闭;阶段文案 + Gauge
  (青色)+ 灰色提示行;
