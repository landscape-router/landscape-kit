...
被忽略（Ctrl+C 仍退出），结果写入底栏；成功后状态行在下一帧自动变为运行中。控制台
不另起 lkit 进程、不解析 CLI 文本输出。进入控制台时若 root 下 daemon 未运行，底栏
提示行也直接显示同样的警告。控制台 header 在品牌标题右侧展示两个状态徽标
...
（窄终端下英文长提示可能换行到第二行），不留空行。右下角持续显示当前语言。非文本编辑和非退出
