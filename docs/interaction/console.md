...
右栏常驻显示 lkit 常驻服务小节：小节标题下带一行灰色简介（说明该服务以 systemd
常驻、代本控制台执行安装/更新/重初始化等特权操作，按栏宽预折行），版本号（当前
二进制版本，daemon 未运行时也显示）
...
提供"`> [ 查看急救恢复码 ]`"动作行（上方一行灰色简介说明急救恢复码用于常规网络
失联时的 L2 flare 急救通道），Enter/空格弹出「查看/修改急救恢复码」
...
服务 ]”按钮（`D` 键，按钮常显选中态）：打开与 Overview 相同的部署确认弹窗
...
控制台为纯键盘交互，不支持鼠标（不启用鼠标捕获，终端鼠标事件被忽略）。所有操作
（菜单切换、表单编辑、确认层、滚动）都通过方向键、Enter/Esc、Space 与快捷键完成，
行为以各面板文档为准。

侧栏和表单使用方向键移动：Right 或 Enter 从侧栏进入面板，Esc 从任意面板（包括
Install）返回侧栏菜单选择；没有左右切换语义的面板（Overview、Backup、Mirror、
Software 等）按 Left 返回侧栏导航，按 Right 无动作。有左右切换语义的场合保留
Left/Right 切换值（Install 与 Update 的仓库枚举、软件来源与镜像确认层内的循环
切换等），其余字段上 Left/Right 不改变焦点、也不跳回侧栏。Install 使用 Up/Down 在
检查汇总和表单字段间移动，检查汇总态按 Enter 或 Right 展开检查详情（Esc 收起），
表单字段上 Right/Enter/Space 切换枚举和开关；Backup 详情页内 Left 由详情视图
消费，仅 Esc 返回列表。Tab 在侧栏与面板间切换，Enter 编辑
...
（窄终端下英文长提示可能换行到第二行），不留空行。状态行按消息级别染色：就绪灰、
过程信息/引导提示黄、操作成功绿、失败/阻断红（`console::notice::Notice` 类型，
取代旧的"非 Ready 一律红字"约定）；换源/恢复等多行结果在同一状态行内按行拼接，
保持首行的级别染色。右下角语言指示在可切换时显示
**目标语言**（以目标语言自身文字书写，如英文界面下的 `[L] Switch to 中文 (zh)`），
按 `L` 即切换到所示目标，所见即所得；退出确认层等不可切换状态退回显示
当前语言（如 `Language: English (en)`），文本编辑中则显示当前语言并解释 `L` 暂停
（`Language: English (en) (L paused while typing)` / `语言：中文 (zh)（输入时暂停 L）`），
避免用户误以为快捷键失效。编辑字段时的
`l` 保持为普通输入字符；内嵌急救恢复码输入的弹窗（部署确认、查看/修改、flare
恢复通道）打开期间 `L` 整体暂停——它们承诺"直接输入即进入编辑"，首字符恰好是
`l` 时也必须落入字段。各页面样式验收标准见 [控制台样式验收标准](ui/README.md)。
