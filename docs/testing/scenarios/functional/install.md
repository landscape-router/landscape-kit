...
- 测试层：双架构 Docker E2E
- 状态：`已覆盖`
- 证据：[Docker CI 边界](../../docker-lifecycle.md#本地运行)
...
- 测试层：Rust workflow、Docker E2E
- 状态：`已覆盖`
- 证据：[下载与发布目录](../../../repository.md#下载与发布目录)、[`release/artifacts.rs`](../../../../lkit-cli/src/release/artifacts.rs)、[Docker E2E S14](../../../docker-lifecycle.md#场景)
...
**`config.toml` 是用户维护配置：仓库来源永不写回，安装不创建该文件**
...
- 说明：首次安装完成后断言 `install-state.json` 不包含 `repository` 字段且 lkit 地盘顶层
  ...
  `config.toml` 只由显式的偏好/通道配置动作写回（`[ui] language` 切换、`lkit self
  install --flare-psk-file`/daemon 自动生成的 `[flare]`、`lkit flare setup`）。
...
**首次安装完成提示 flare 恢复通道就绪（psK 由 daemon 侧管理）**

- 测试层：CLI fixture E2E、单元测试
- 状态：`已覆盖`
- 证据：[flare 部署形态](../../../flare/protocol.md#部署形态)、[SS-10](self.md#ss-10)
- 说明：急救恢复码属于 daemon 配置，由 `lkit self install`（`--flare-psk-file` / 交互
  提示 / daemon 自动生成）与 `lkit flare setup` 管理，`lkit install` 不要求、不写入
