...
见 [`migrate.rs`](../../../../crates/lkit-cli/tests/install_fixture_e2e/migrate.rs)。
...
- 状态：`已覆盖`
- 证据：[migrate 命令](../../commands/migrate.md)、`migrates_in_systemd_mode_with_legacy_unit_adoption`（crates/lkit-cli/src/workflows/migrate/tests.rs）
...
- 测试层：核心功能（E2E + 单元测试，fake systemctl）
- 状态：`已覆盖`
- 证据：[migrate 命令](../../commands/migrate.md#失败与恢复)、`migrate_rolls_back_and_restores_legacy_unit_on_activation_failure`（crates/lkit-cli/tests/install_fixture_e2e/migrate.rs）、`systemd_mode_rolls_back_and_restores_legacy_unit_on_activation_failure`（crates/lkit-cli/src/workflows/migrate/tests.rs）
