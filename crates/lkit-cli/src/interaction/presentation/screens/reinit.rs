use super::{OperationPhase, OperationResult, OperationScreen};
...
pub(crate) struct ReinitScreen;

impl OperationScreen for ReinitScreen {
    fn result_key(&self, result: OperationResult) -> &'static str {
        match result {
            OperationResult::Success => crate::keys::PRESENTATION_REINIT_COMPLETE,
            OperationResult::Failed => crate::keys::PRESENTATION_REINIT_FAILED,
            OperationResult::Cancelled => crate::keys::PRESENTATION_REINIT_CANCELLED,
        }
    }
    ...
    fn render(
        ...
        current: Option<&super::DownloadState>,
        logs: &[String],
        notice: &str,
        ...
        let header_text = match result {
            Some(OperationResult::Success) => crate::tr!(crate::keys::PRESENTATION_REINIT_COMPLETE),
            Some(OperationResult::Failed) => crate::tr!(crate::keys::PRESENTATION_REINIT_FAILED),
            Some(OperationResult::Cancelled) => {
                crate::tr!(crate::keys::PRESENTATION_REINIT_CANCELLED)
            }
            None => crate::tr!(crate::keys::PRESENTATION_OPERATION_REINIT),
        };
        frame.render_widget(
...
mod tests {
        ...
        terminal
            .draw(|frame| {
                ReinitScreen.render(
                    ...
                    step_progress,
                    None,
                    &["reinit: activated version 0.19.2; awaiting network confirmation".into()],
                    "",
                    false,
                    result,
                )
    ...
    #[test]
    fn renders_reinit_result_with_its_own_title_not_installation_wording() {
        let content = render(
        ...
        assert!(
            content.contains("Reinit complete"),
            "the reinit result page must not reuse the installation title"
        );
    ...
    #[test]
    fn reinit_in_progress_hint_is_not_installation_wording() {
        let content = render(OperationPhase::Verifying, None, None);
        assert!(content.contains("Reinitializing Landscape"));
        assert!(
            content.contains("The operation is in progress; stop requests are ignored"),
            "the reinit page hint must not reuse the installation wording"
        );
    ...
    fn renders_step_progress_gauge_for_stepped_operations() {
        let content = render(OperationPhase::Stopping, Some((2, 4)), None);
        assert!(content.contains("Reinitializing Landscape"));
