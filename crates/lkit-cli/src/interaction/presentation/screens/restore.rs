...
impl OperationScreen for RestoreScreen {
    fn result_key(&self, result: OperationResult) -> &'static str {
        match result {
            OperationResult::Success => crate::keys::PRESENTATION_RESTORE_COMPLETE,
            OperationResult::Failed => crate::keys::PRESENTATION_RESTORE_FAILED,
            OperationResult::Cancelled => crate::keys::PRESENTATION_RESTORE_CANCELLED,
        }
    }
    ...
    fn render(
        ...
        let header_text = match result {
            Some(OperationResult::Success) => {
                crate::tr!(crate::keys::PRESENTATION_RESTORE_COMPLETE)
            }
            Some(OperationResult::Failed) => {
                crate::tr!(crate::keys::PRESENTATION_RESTORE_FAILED)
            }
            Some(OperationResult::Cancelled) => {
                crate::tr!(crate::keys::PRESENTATION_RESTORE_CANCELLED)
            }
            None => crate::tr!(crate::keys::PRESENTATION_OPERATION_RESTORE),
        };
        frame.render_widget(
...
mod tests {
    ...
    fn render(
        ...
        terminal
            .draw(|frame| {
                RestoreScreen.render(
                    frame,
                    phase,
                    step_progress,
                    None,
                    &["restore: restored to version 0.22.2 from backup x".into()],
                    "",
                    false,
                    result,
                )
    ...
    #[test]
    fn renders_restore_result_with_its_own_title_not_install_wording() {
        let content = render(
        ...
        assert!(
            content.contains("Restore complete"),
            "the restore result page must not reuse the installation title"
        );
    ...
    #[test]
    fn restore_in_progress_hint_is_not_installation_wording() {
        ...
        assert!(
            ...
            "the restore page hint must not reuse the installation wording"
        );
