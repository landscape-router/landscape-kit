...
mod tests {
    ...
    use super::super::tests::{
        ...
        activate_version, create_target_backup, export_server, install_state, interactive_guard,
        none_health, setup_current, temp_root,
    };
    ...
    async fn none_mode_activation_failure_is_recovered_by_next_command() {
        ...
        let unfinished = find_unfinished(&install_root)
            .unwrap()
            .expect("failed none-mode restore must leave an unfinished transaction");
        assert_eq!(unfinished.phase, Phase::Activating);
        ...
        recover_interrupted(&install_root, &unfinished, &Systemd::host(), &none_health())
            .await
            .unwrap();
